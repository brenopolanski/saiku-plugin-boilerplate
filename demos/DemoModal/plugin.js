 ...
 * Under MIT License
 */
var DemoModal = Backbone.View.extend({
	...
	add_button: function() {
		var button =
			$('<a href="#demoModal" class="demoModal button disabled_toolbar i18n" title="Demo Modal"></a>')
			.css({ 'background-image': 'url("js/saiku/plugins/DemoModal/image/plugin.png")',
		...
		this.workspace.toolbar.demoModal = this.show;
	},
...
Saiku.events.bind('session:new', function() {
	...
	// loadCSS('js/saiku/plugins/DemoModal/css/plugin.css');	
	// loadJS('js/saiku/plugins/DemoModal/js/lib.js');

	function new_workspace(args) {
		if (typeof args.workspace.demoModal === 'undefined') {
			args.workspace.demoModal = new DemoModal({ workspace: args.workspace });
		}
	}
